...
describe('Invalid clientCredential on authServer endpoint', () => {
  test.each([{ client_id: 'wrong', client_secret: VALID_CLIENT_SECRET },
...
describe('Valid clientCredentials, but mandatory fields missing on authServer endpoint', () => {
  const validCredentials = { client_id: VALID_CLIENT_ID, client_secret: VALID_CLIENT_SECRET }
  test.each([{},
    { scope: 'wrong-scope' },
    { scope: VALID_SCOPE },
    { scope: VALID_SCOPE, grant_type: 'wrong-grant' }
  ])('Incomplete request %p gives 400', (requestFields, done) => {
    sendFormRequest(AUTH_SERVER_URL,
...
describe('Valid token from authServer', () => {
  ...
  beforeAll(async () => {
    ...
    accessToken = (await sendFormRequest(AUTH_SERVER_URL, validRequest, null, 200, null)).body
    validateAccessToken(accessToken)
  })

  test('Get a valid FCM token', async () => {
    const fcmToken = (await sendFormRequest(FCM_TOKENS_URL,
      { grant_type: VALID_GRANT, fcm_project_number: VALID_FCM_PROJECT_NUMBER },
  ...
  test.each([[{}, 401],
    ...
    [{ fcm_project_number: VALID_FCM_PROJECT_NUMBER, grant_type: 'wrong' }, 400]
  ])('Request %p to FCM token endpoint gives %p', (body, expectedResult, done) => {
    sendFormRequest(FCM_TOKENS_URL, body, accessToken.access_token, expectedResult, done)
  })
})
...
async function sendFormRequest (endpoint, body, bearerToken, expectedResponseCode, done) {
  let req = request(app).post(endpoint)
  if (bearerToken) { req = req.set('Authorization', `Bearer ${bearerToken}`) }
  req = req.type('form')
